...
| `focus_point` | An array of objects in the format `{ x: 0.6, y: 0.4 }` where `x` is the focus point offset from the left and `y` - from the top. Each focus point corresponds to an image from the array of images and specifies a part of the picture that should stay visible when the viewport height or width change and the picture gets cropped. If specified, overrides `centeredX` and `centeredY`. | Object | 0, 0 |
